...
const GLfloat YAW = -120.0f;   // Y
const GLfloat PITCH = -30.0f;  // X
...
class Camera {
   public:
    ...
    void ProcessMouseMovement(GLfloat xoffset, GLfloat yoffset, GLboolean constraintPitch = GL_TRUE, GLboolean invertYAxis = GL_FALSE) {
        ...
        this->Yaw += xoffset;
        this->Pitch += (invertYAxis) ? -yoffset : yoffset;
...
void apply_camera_movements(Camera& camera, bool (&keys)[1024], GLfloat deltaTime) {
    // if a single WASD key is pressed, then we will apply the full value of velocity v in the corresponding direction.
    // However, if two keys are pressed together in order to move diagonally (W+D, W+A, S+D, S+A),
